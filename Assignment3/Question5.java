...
public class Question5 {
    ...
    public static class TopNMapper extends Mapper<Object, Text, Text, DoubleWritable> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            String[] reviewData = StringUtils.split(value.toString(),"^");
            //StringTokenizer itr = new StringTokenizer(cleanLine);
            /*while (itr.hasMoreTokens()) {
            	String token=itr.nextToken();
            	if(token.matches("[1-9][0-9]{4}")){
            		//word.set(itr.nextToken().trim());
            		word.set(token);
            		context.write(word, one);
            	}
            }*/
            //while(reviewData!=null){
            	if (reviewData.length ==4) {
				
					word.set(reviewData[2]);
					//Integer rating=(int)Float.parseFloat(reviewData[3]);
					context.write(word, new DoubleWritable(Double.parseDouble(reviewData[3])));//not working ;numberformatexception
					//context.write(word, new FloatWritable(Float.parseFloat(reviewData[3])));//not working ;numberformatexception
    ...
    public static class TopNReducer extends Reducer<Text, DoubleWritable, Text, DoubleWritable> {
        ...
        public void reduce(Text key, Iterable<DoubleWritable> values, Context context) throws IOException, InterruptedException {
            ...
            countMap.put(new Text(key), new DoubleWritable(sum/i));
        }
